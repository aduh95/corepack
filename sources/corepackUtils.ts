...
export async function installVersion(installTarget: string, locator: Locator, {spec}: {spec: PackageManagerSpec}): Promise<InstallSpec> {
  ...
  if (!build[1] && !canVerifySignature) {
    const {COREPACK_ON_UNVERIFIED_DOWNLOAD} = process.env;
    debugUtils.log(`No hash provided, and signature cannot be verified; checking COREPACK_ON_UNVERIFIED_DOWNLOAD, set to: ${COREPACK_ON_UNVERIFIED_DOWNLOAD}`);
  ...
  if (build[1] && actualHash !== build[1]) {
    await fs.promises.rm(tmpFolder, {recursive: true, force: true}).catch(console.warn);
